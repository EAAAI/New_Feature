...
<template>
    <div
        v-if="show"
        :dir="uiDir"
        class="mb-4 flex items-center justify-between gap-3 rounded-md border border-border bg-accent px-3 py-2 text-sm text-primary shadow-sm"
    >
        <span>{{ t('builder.languageMismatch', { language: resumeLanguageName }) }}</span>
        <button
            class="rounded p-1 text-primary hover:bg-accent/80"
